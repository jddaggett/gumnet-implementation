...
def get_transformation_output_from_model(model, x_test, y_test, device, batch_size=32):
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(x_test), batch_size):
            ...
            _, params = model(x_batch, y_batch)
...
def main(DEBUG=False, batch_size=32, initial_lr=1e-4, AUGMENT=False, scheduler_enabled=True, random_initialization=True):
    ...
    for i in range(20):
        ...
        for x_batch, y_batch, gt_batch in dataloader:
            ...
            optimizer.zero_grad()
            output, params = model(x_batch, y_batch)

            # calculate loss directly with 6D transformation parameters
            loss = correlation_coefficient_loss(output, y_batch)
...
if __name__ == '__main__':
    main(DEBUG=False, batch_size=32, initial_lr=5e-5, AUGMENT=True, scheduler_enabled=True ,random_initialization=True)
